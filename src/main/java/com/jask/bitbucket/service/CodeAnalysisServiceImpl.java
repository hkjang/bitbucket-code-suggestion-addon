...
import com.jask.bitbucket.security.SecretMasker;
...
public class CodeAnalysisServiceImpl implements CodeAnalysisService {
    ...
    @Inject
    public CodeAnalysisServiceImpl(LlmClientService llmClient,
    ...
    public AnalysisResponse analyze(AnalysisRequest request) {
        ...
        try {
            ...
            AnalysisRequest.AnalysisOptions options = request.getOptions();

            List<CodeSuggestion> filteredSuggestions = allSuggestions.stream()
                    .filter(s -> s.getConfidence() >= minConfidence)
                    .filter(s -> isCategoryEnabled(s.getCategory(), options))
    ...
    public AnalysisResponse analyzeFile(AnalysisRequest.FileDiff fileDiff, String language) {
        AnalysisResponse response = new AnalysisResponse();
        String model = settingsService.getLlmModel();

        // 캐시 조회
        String diffContent = fileDiff.getDiff() != null ? fileDiff.getDiff() : "";
        String cachedResponse = cacheService.get(diffContent, model);

        String llmResponse;
        if (cachedResponse != null) {
            log.debug("캐시 히트: {}", fileDiff.getFilePath());
            ...
            // 캐시 저장
            cacheService.put(diffContent, model, llmResponse);
        }
    ...
    private List<AnalysisRequest.FileDiff> filterFiles(List<AnalysisRequest.FileDiff> fileDiffs) {
        ...
        int maxFileSizeKb = settingsService.getMaxFileSizeKb();

        return fileDiffs.stream()
                .filter(f -> !isExcluded(f.getFilePath(), excludedSet))
                .filter(f -> isSupportedLanguage(f.getFilePath()))
                .filter(f -> isWithinSizeLimit(f, maxFileSizeKb))
                .limit(maxFiles)
                .collect(Collectors.toList());
    }

    private boolean isWithinSizeLimit(AnalysisRequest.FileDiff fileDiff, int maxSizeKb) {
        String diff = fileDiff.getDiff();
        String fullContent = fileDiff.getFullContent();
        long totalBytes = (diff != null ? diff.length() : 0) + (fullContent != null ? fullContent.length() : 0);
        long totalKb = totalBytes / 1024;
        if (totalKb > maxSizeKb) {
            log.info("파일 크기 제한 초과로 건너뜀: {} ({}KB > {}KB)", fileDiff.getFilePath(), totalKb, maxSizeKb);
            return false;
        }
        return true;
    }
    ...
    private String buildUserPrompt(AnalysisRequest.FileDiff fileDiff, String language) {
        ...
        // 시크릿 마스킹 적용 후 전송
        String maskedDiff = SecretMasker.mask(fileDiff.getDiff());
        sb.append("## Diff 내용\n\n```diff\n");
        sb.append(maskedDiff);
        sb.append("\n```\n\n");

        if (fileDiff.getFullContent() != null && !fileDiff.getFullContent().isEmpty()) {
            String maskedContent = SecretMasker.mask(fileDiff.getFullContent());
            sb.append("## 전체 파일 컨텍스트 (참고용)\n\n```").append(language).append("\n");
            sb.append(maskedContent);
            sb.append("\n```\n\n");
        }
    ...
    private List<CodeSuggestion> parseSuggestions(String llmResponse, String filePath) {
        ...
        try {
            ...
            JsonArray jsonArray = new JsonParser().parse(new StringReader(jsonContent)).getAsJsonArray();
    ...
    /**
     * AnalysisOptions에 따라 카테고리가 활성화되어 있는지 확인합니다.
     */
    private boolean isCategoryEnabled(CodeSuggestion.Category category, AnalysisRequest.AnalysisOptions options) {
        if (options == null || category == null) {
