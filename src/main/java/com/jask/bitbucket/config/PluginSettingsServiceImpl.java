...
public class PluginSettingsServiceImpl implements PluginSettingsService {
    ...
    public String getLlmApiKey() {
        String stored = get("llm.apiKey", "");
        if (stored.isEmpty()) {
            return stored;
        }
        try {
            return CredentialEncryptor.decrypt(stored);
        } catch (Exception e) {
            log.warn("API 키 복호화 실패, 평문으로 반환 (마이그레이션 필요): {}", e.getMessage());
            return stored;
        }
    }
    ...
    public void setLlmApiKey(String apiKey) {
        ...
        set("llm.apiKey", CredentialEncryptor.encrypt(apiKey));
    }
